...
from pic1.util import save_plot, save_data, load_data
from pic1.weighting import weight_particles
...
def calc_growth_rate(
    ...
    if not headless:
        ...
        fig.suptitle(
            f"$k={k}$, $k v_0 /\omega_p={k*v0/c.wp:.2f}$, $M={M}$, $N={c.N}$"
        )
        ...
        ax_field.plot(x_j_unorm, m.d.initial_ex)
        ax_field.set_xlim(c.x_min, c.x_max)
    ...
    if demo_mode:
        print("Using stale data in demo.")
    else:
        m.run()
        save_data(m, "two-beam-data")
    d = m.d
    t_steps = c.t_steps
    ke_hist = d.ke_hist
    fe_hist = d.fe_hist
    ...
    if not headless:
        plots.animate_phase_space(
            m,
            plot_title=(
                f"Two stream instability: $k={k}$, $k*v_0/\omega"
                f" _p={k*v0/wp:.2f}$, $\delta x={c.perturbation}$"
            ),
            repeat=True,
            hold=True,
        )
...
if __name__ == "__main__":
    ...
    if not demo_mode:
        ...
        with multiprocessing.Pool(
            min(len(k_trials), multiprocessing.cpu_count() - 2)
        ) as p:
